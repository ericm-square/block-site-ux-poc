...
const utils = require('./utils-8cc937f5.js');
...
const identity = require('./identity-8189b2e1.js');
const _Map = require('./_Map-07e1b6b7.js');
const _getTag = require('./_getTag-7be0e78a.js');
...
function get(object, path, defaultValue) {
  var result = object == null ? undefined : hasIn.baseGet(object, path);
...
function baseMatchesProperty(path, srcValue) {
  if (hasIn.isKey(path) && isStrictComparable(srcValue)) {
    return matchesStrictComparable(hasIn.toKey(path), srcValue);
  }
  return function(object) {
    var objValue = get(object, path);
    return (objValue === undefined && objValue === srcValue)
      ? hasIn.hasIn(object, path)
...
function basePropertyDeep(path) {
  return function(object) {
    return hasIn.baseGet(object, path);
  };
}
...
function property(path) {
  return hasIn.isKey(path) ? baseProperty(hasIn.toKey(path)) : basePropertyDeep(path);
}
...
function baseUniq(array, iteratee, comparator) {
  ...
  if (comparator) {
    ...
    isCommon = false;
    includes = _baseIsEqual.cacheHas;
    seen = new _baseIsEqual.SetCache;
  }
  else {
...
const MarketList = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.marketListSelectionsDidChange = index.createEvent(this, "marketListSelectionsDidChange", 7);
        this.marketListSlotChange = index.createEvent(this, "marketListSlotChange", 7);
        this.marketListItemsReordered = index.createEvent(this, "marketListItemsReordered", 3);
        this.marketListItemsFiltered = index.createEvent(this, "marketListItemsFiltered", 7);
        this.observers = {};
        this.value = '';
    ...
    getEventSelectionDetails() {
        const prevSelectionValues = (() => {
            if (typeof this.value === 'string') {
                return this.value ? this.value.split(',').filter((value) => !utils.isValueEmpty(value)) : [];
            }
            else if (Array.isArray(this.value)) {
                return this.value.filter((value) => !utils.isValueEmpty(value));
            }
            return this.value;
        })();
        ...
        const currentSelectionValues = currentSelections.reduce((items, item) => {
            if (!utils.isValueEmpty(item.value)) {
    ...
    syncListInteractiveWithItems() {
        if (this.items.length > 0 &&
            (this.items[0].tagName === index$1.getNamespacedTagFor('market-action-card').toUpperCase() ||
                (this.items[0].tagName === index$1.getNamespacedTagFor('market-row').toUpperCase() &&
    ...
    processItems() {
        this.items.forEach((item) => {
            if (item.tagName === index$1.getNamespacedTagFor('market-action-card').toUpperCase()) {
                item.transient = this.transient;
                const cardRow = item.querySelector(index$1.getNamespacedTagFor('market-row'));
                if (cardRow) {
    ...
    setSelectionsFromValue() {
        ...
        if (this.value || this.value === '') {
            ...
            (_a = this.items) === null || _a === void 0 ? void 0 : _a.forEach((item) => {
                ...
                if (!utils.isValueEmpty(this.value) && values.has(item.value)) {
                    item.silentlySelect();
    ...
    setListAndItemsRoles() {
        const itemSelectableTypes = this.items.map((item) => utils.getItemSelectableType(item));
        const allItemsHaveSameSelectableType = itemSelectableTypes.every((type) => type === utils.ListItemSelectableType.CHECKBOX) ||
            itemSelectableTypes.every((type) => type === utils.ListItemSelectableType.RADIO) ||
            itemSelectableTypes.every((type) => type === utils.ListItemSelectableType.SWITCH) ||
            itemSelectableTypes.every((type) => type === utils.ListItemSelectableType.SELECTABLE_WITHOUT_CONTROL);
    ...
    setInternalState() {
        this.items = [
            ...this.el.querySelectorAll(`:scope > ${index$1.getNamespacedTagFor('market-row')}, :scope > ${index$1.getNamespacedTagFor('market-action-card')}`),
        ];
        if (this.items.length === 0) {
            return;
        }
    ...
    async syncControlRowWithSelections() {
        ...
        const isCheckbox = slottedControl.tagName.toLowerCase() === index$1.getNamespacedTagFor('market-checkbox');
        const items = ((_b = (_a = this.filteredItems) === null || _a === void 0 ? void 0 : _a.visible) !== null && _b !== void 0 ? _b : this.items).filter((item) => item !== this.controlRow);
        ...
        switch (controlRowStatus) {
            case 'unchecked':
                ...
                await raf.asyncRequestAnimationFrame(); // prevents flash where it goes from indeterminate -> checked -> unchecked
            ...
            case 'indeterminate':
                ...
                if (isCheckbox) {
                    ...
                    await this.controlRow.silentlySelect();
                    this.selectItem(this.controlRow);
                    await raf.asyncRequestAnimationFrame(); // prevents bug where checkbox becomes checked but not indeterminate
    ...
    controlRowSlotchangeHandler() {
        ...
        this.controlRow = utils.isValidControl(slottedControl) ? slottedRow : undefined;
    }
    /**
     * Show empty state if:
    ...
    setEmptyStateVisibility() {
        ...
        const emptyStateEl = this.el.querySelector('[slot="empty-state"]') ||
            this.el.shadowRoot.querySelector(index$1.getNamespacedTagFor('market-empty-state'));
    ...
    */
    ...
    setReorderable() {
        const { el, items, controlRow, reorderable: reorderable$1, reorderMode, reorder, marketListItemsReordered } = this;
        if (reorderable$1 === 'off') {
            ...
            this.reorder = new reorderable.Reorderable({
                el,
                accepts: [`${index$1.getNamespacedTagFor('market-row')}:not([slot="control"])`],
                event: marketListItemsReordered,
                mode: reorderMode,
            });
        }
        items === null || items === void 0 ? void 0 : items.forEach((item) => {
            if (!draggable.isDraggable(item))
                return;
            if (item === controlRow)
                return; // control row is not reorderable
            item.dragEnabled = reorderable$1 !== 'off';
        });
    }
    onDragMove(e) {
        var _a;
    ...
    render() {
        var _a;
        const MarketEmptyState = index$1.getNamespacedTagFor('market-empty-state');
        return (index.h(index.Host, { key: 'f01372045695a9776c6318303fecd322561cdb1d', class: "market-list", "aria-labelledby": this.name, "has-search": this.hasSearch, onKeydown: (e) => this.handleKeydown(e), onMarketDragMove: (e) => this.onDragMove(e), onMarketDragLeave: () => this.onDragLeave(), onMarketDragEnd: (e) => this.onDragEnd(e), onMarketDragDrop: (e) => this.onDragDrop(e) }, index.h("slot", { key: 'e5a0f3a54f00422e2a116d5958a1d733d4aea540', name: "search", onSlotchange: () => this.handleSearchSlotchange() }), index.h("slot", { key: '3aaca12e5456bc8df62bf982d00a997ffdd6a6d8', name: "control-row", onSlotchange: () => this.controlRowSlotchangeHandler() }), index.h("slot", { key: '2b528867d3784890f0f54619b302c223eee4cdd5', onSlotchange: () => this.defaultSlotchangeHandler() }), index.h("slot", { key: '7d6f8a7cf5a9add9811afe49fe24621260887769', name: "empty-state" }, index.h(MarketEmptyState, { key: 'af825be4ce90fa63ec83b1f2fda05abb47401750', class: "hidden" }, index.h("svg", { key: '9088bd651dca4d4beaaf6b22e1917af118767e21', height: "40", slot: "media", viewBox: "0 0 40 40", width: "40", xmlns: "http://www.w3.org/2000/svg" }, index.h("path", { key: '795ff6d2afc5584c7b516e60fa385af8964e24be', d: "M34.4667 17.2L28.1 10.8333H26.6667C26.6667 9.45 25.55 8.33333 24.1667 8.33333C22.7834 8.33333 21.6667 9.45 21.6667 10.8333H18.3334C18.3334 9.45 17.2167 8.33333 15.8334 8.33333C14.45 8.33333 13.3334 9.45 13.3334 10.8333H11.9L5.53337 17.2C4.11671 18.6167 3.33337 20.5 3.33337 22.5C3.33337 26.6333 6.70004 30 10.8334 30C14.8 30 18.0167 26.9 18.2834 23.0167C18.8167 23.2167 19.4 23.3333 20 23.3333C20.6 23.3333 21.1834 23.2167 21.7167 23.0167C21.9834 26.9 25.2 30 29.1667 30C33.3 30 36.6667 26.6333 36.6667 22.5C36.6667 20.5 35.8834 18.6167 34.4667 17.2ZM10.8334 26.6667C8.53337 26.6667 6.66671 24.8 6.66671 22.5C6.66671 21.3833 7.10004 20.3333 7.88337 19.55C8.66671 18.7667 9.71671 18.3333 10.8334 18.3333C13.1334 18.3333 15 20.2 15 22.5C15 24.8 13.1334 26.6667 10.8334 26.6667ZM15.35 16.55C14.4667 15.8833 13.4334 15.3833 12.3 15.15L13.2667 14.1667H17.2334C16.3834 14.7333 15.7167 15.5667 15.35 16.55ZM20 20C19.0834 20 18.3334 19.25 18.3334 18.3333C18.3334 17.4167 19.0834 16.6667 20 16.6667C20.9167 16.6667 21.6667 17.4167 21.6667 18.3333C21.6667 19.25 20.9167 20 20 20ZM22.75 14.1667H26.7167L27.7 15.15C26.5667 15.3833 25.5334 15.8833 24.65 16.55C24.2834 15.5667 23.6167 14.7333 22.75 14.1667ZM29.1667 26.6667C26.8667 26.6667 25 24.8 25 22.5C25 20.2 26.8667 18.3333 29.1667 18.3333C30.2834 18.3333 31.3334 18.7667 32.1167 19.55C32.9 20.3333 33.3334 21.3833 33.3334 22.5C33.3334 24.8 31.4667 26.6667 29.1667 26.6667Z", fill: "var(--core-text-10-color)", "fill-opacity": "0.9" })), index.h("h3", { key: '2ed929f1c74989aa9df9809fa509f8c037d05d9b', slot: "primary-text" }, index.h("slot", { key: 'eb5ab0008deb0ca384e32a02448f72e836837b2a', name: "empty-state-primary-text" }, "No search results for \u201C", (_a = this.inputSearchEl) === null || _a === void 0 ? void 0 :
            _a.value, "\u201D")), index.h("p", { key: 'd0cb3833586c6b08307cb3529876a44d3668be4c', slot: "secondary-text" }, index.h("slot", { key: '690ffc4bb1f6c81d12ce44aca5aea14d1e8c1bec', name: "empty-state-secondary-text" }, "Try a different search."))))));
    }
...
MarketList.style = MarketListStyle0;

exports.market_list = MarketList;

//# sourceMappingURL=market-list.cjs.entry.js.map