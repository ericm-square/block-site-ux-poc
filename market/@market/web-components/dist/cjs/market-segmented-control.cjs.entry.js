...
const MarketSegmentedControl = class {
    constructor(hostRef) {
        ...
        this.marketSegmentedSelectionDidChange = index.createEvent(this, "marketSegmentedSelectionDidChange", 7);
     ...
     */
    setInternalState() {
        this.items = this.el.querySelectorAll(`${index$1.getNamespacedTagFor('market-segment')}`);
    ...
    setSliderSize() {
        var _a;
        this.el.style.setProperty('--selected-slider-width', `calc(${100 / ((_a = this.items) === null || _a === void 0 ? void 0 : _a.length)}% - ${index$2.cjs.SEGMENTED_CONTROL_PADDING_HORIZONTAL * 2}px)`);
    }
    /**
    ...
    setSliderPosition(selectedItem = undefined) {
        if (!selectedItem) {
            this.el.style.setProperty('--selected-slider-left', `${index$2.cjs.SEGMENTED_CONTROL_PADDING_HORIZONTAL}px`);
            return;
        }
        const newSelectionIndex = [...this.items].indexOf(selectedItem);
        this.el.style.setProperty('--selected-slider-left', `calc(${(newSelectionIndex / this.items.length) * 100}% + ${index$2.cjs.SEGMENTED_CONTROL_PADDING_HORIZONTAL}px)`);
    }
    /**
     * Sets the initial state of the segmented-control by updating and propagating props and setting
     * default value.
    ...
    marketSegmentSelectedEventHandler(e) {
        ...
        const prevSelection = this.el.querySelector(`${index$1.getNamespacedTagFor('market-segment')}[selected]`);
    ...
    render() {
        return (index.h(index.Host, { key: '9b4e2e05d111b43b02ad003f5ff57f9f65afa610', class: "market-segmented-control" }, index.h("slot", { key: '25608a3e24e8bfff6d9683db3a92553cffb896e0', onSlotchange: () => this.defaultSlotchangeHandler() })));
    }
...
exports.market_segmented_control = MarketSegmentedControl;

//# sourceMappingURL=market-segmented-control.cjs.entry.js.map