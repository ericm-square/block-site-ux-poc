...
const Popover = class {
    ...
    initInteractiveList() {
        ...
        let lists;
        if (slot) {
            lists = slot.assignedElements().filter((el) => el.localName === index$1.getNamespacedTagFor('market-list'));
        }
        else {
            /* .querySelectorAll() returns a NodeList and not an array so we need to convert it
             in order to use .map() below */
            lists = [].slice.call(this.el.querySelectorAll(index$1.getNamespacedTagFor('market-list')));
        }
        if (lists) {
            lists.forEach((list) => {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '8eabe587b426e91c81a0739af9800955a5c31640', class: "market-popover" }, index.h("slot", { key: 'a7f140e994d37aac481bbde312d2cd2604ab3e12' })));
    }
    get el() { return index.getElement(this); }
};
Popover.style = MarketPopoverStyle0;

exports.market_popover = Popover;

//# sourceMappingURL=market-popover.cjs.entry.js.map